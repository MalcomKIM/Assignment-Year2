...
public class HttpInteract {
	...
	private int bytesRead = 0;		//bytes read in body
	private String contentType;		//content type of the URL 
	...
	public String send() throws IOException {
		...
		if (status == 301 || status == 302) {
			/* If the status code equals 301 or 302, read the response header to
			 * extract the new location and return it*/
			String temp = null;
			String newurl="";
			
			while ((temp = fromServer.readLine()).length() != 0) {
				headers += temp + "\n";
				if (temp.startsWith("Location: http://")) {
					newurl=temp.substring(17);
				}
			}
			...
			if (bodyLength != -1) {
				//bytesRead = fromServer.read(body);
				
				int times=bodyLength/BUF_SIZE+1;
				for(int i=0;i<times;i++) {
					buf = new char[BUF_SIZE];
					bytesRead=bytesRead+fromServer.read(buf);
	...
	private char[] read_chunk(BufferedReader fromServer,char[] buf,char body[]) throws IOException {
		...
		while (true) {
			...
			bytesRead += size;
			System.arraycopy(buf, 0, body, bytesRead, size);
	...
	//For other classes to get status code from server
	public int getStatusCode() {
		return status;
	}
	
	//for other classes to get content type of URL
